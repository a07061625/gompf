...
func (tc *tradeCancel) SetOutTradeNo(outTradeNo string) {
...
func (tc *tradeCancel) SetTradeNo(tradeNo string) {
...
func (tc *tradeCancel) CheckData() (*fasthttp.Client, *fasthttp.Request) {
    if len(tc.tradeNo) > 0 {
...
func NewTradeCancel(appId string) *tradeCancel {
    tc := &tradeCancel{alipay.NewBase(appId), "", ""}
    tc.SetMethod("alipay.trade.cancel")
    return tc
}
