/**
 * Created by GoLand.
 * User: 姜伟
 * Date: 2020/2/2 0002
 * Time: 9:29
 */
package domain

import (
    "regexp"

    "github.com/a07061625/gompf/mpf/api/mpiot"
    "github.com/a07061625/gompf/mpf/mpconstant/errorcode"
    "github.com/a07061625/gompf/mpf/mpconstant/project"
    "github.com/a07061625/gompf/mpf/mperr"
    "github.com/valyala/fasthttp"
)

// 获取权限组接入信息
type accessDetail struct {
    mpiot.BaseBaiDu
    domainName string // 权限组名称
}

func (ad *accessDetail) SetDomainName(domainName string) {
    match, _ := regexp.MatchString(project.RegexDigitAlpha, domainName)
    if match {
        ad.domainName = domainName
    } else {
        panic(mperr.NewIotBaiDu(errorcode.IotBaiDuParam, "权限组名称不合法", nil))
    }
}

func (ad *accessDetail) CheckData() (*fasthttp.Client, *fasthttp.Request) {
    if len(ad.domainName) == 0 {
        panic(mperr.NewIotBaiDu(errorcode.IotBaiDuParam, "权限组名称不能为空", nil))
    }
    ad.ServiceUri = "/v3/iot/management/domain/" + ad.domainName + "/accessDetail"

    ad.ReqURI = ad.GetServiceUrl()

    return ad.GetRequest()
}

func NewAccessDetail() *accessDetail {
    ad := &accessDetail{mpiot.NewBaseBaiDu(), ""}
    return ad
}
